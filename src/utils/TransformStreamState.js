export default class TransformStreamState {
  ...
  constructor(stream, options) { //eslint-disable-line better/explicit-return
    this.stream = stream;
  ...
  handleChar(c) {
    ...
    if (!this._in_match) {
      ...
      if (expectsAnother === undefined) { // expectation was not met
  ...
  unmatch() {
    this.stream(this.unbuffer(this._sliding === true
      ? 1
      : this._buffer.length,
    ));
